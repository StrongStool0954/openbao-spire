...
func GetSPIREFunc(kms *KMS, opts ...wrapping.Option) (wrapping.Wrapper, map[string]string, error) {
	wrapper := spireseal.NewWrapper()
	wrapperInfo, err := wrapper.SetConfig(context.Background(), append(opts, wrapping.WithConfigMap(kms.Config))...)
	if err != nil {
		return nil, nil, err
	}
	info := make(map[string]string)
	if wrapperInfo != nil {
		info["SPIRE Socket"] = wrapperInfo.Metadata["socket_path"]
		info["SPIRE Trust Domain"] = wrapperInfo.Metadata["trust_domain"]
		info["PKCS#11 Library"] = wrapperInfo.Metadata["pkcs11_lib"]
		if val, ok := wrapperInfo.Metadata["pkcs11_token"]; ok {
			info["PKCS#11 Token"] = val
		}
	}
	return wrapper, info, nil
}
